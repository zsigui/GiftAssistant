package net.youmi.android.libs.common.v2.download.base;//package net.youmi.android.libs.common.v2.download.base;
//
...
// * <p/>
// * {@link DefaultAutoRetryFileDownloader}　这个类对本类进行了封装，支持下载失败后的重试
// *
// * @author zhitao
...
// */
//public class BaseHttpClientFileDownloader implements IDownloader {
//
//	/**
//	 * 每次读入网络流写入到文件的长度(字节)
//	 */
...
//
//	public BaseHttpClientFileDownloader(Context context, FileDownloadTask fileDownloadTask) {
//		mContext = context;
...
//				get = new HttpGet(mFileDownloadTask.getDestDownloadUrl());
//				get.addHeader("HOST", host);
//			}
//			// 创建默认的HttpGet
//			else {
//				get = new HttpGet(mFileDownloadTask.getRawDownloadUrl());
//			}
//
//			// 是否使用断点续传
...
//			int mHttpResponseCode = response.getStatusLine().getStatusCode(); // 获取最终状态码
//
//			// http状态码错误，结束
//			if (mHttpResponseCode < 200 || mHttpResponseCode >= 300) {
//				return new FinalDownloadStatus(FinalDownloadStatus.Code.FAILED_ERROR_HTTP_CODE);
//			}
//
//			// 设置本次任务的最终下载地址
//			setFileDownloadTaskFinalDownloadUrl(httpContext);
//
//			// 这里特别解释一下：
//			// response.getEntity().getContentLength()拿到的是本次http请求的文件部分长度。
...
//				sb.append("\n * http返回状态码:").append(mHttpResponseCode);
...
//	/**
//	 * 因为原始下载地址有可能是302的请求，
//	 * 所以我们需要在发起请求之后,设置下载任务的最终下载地址
//	 *
//	 * @param httpContext httpContext会在execute中传入，记录请求中的一些状态信息，如：记录重定向的信息
//	 */
//	private void setFileDownloadTaskFinalDownloadUrl(HttpContext httpContext) {
...
//		}
//		mFileDownloadTask.setDestDownloadUrl(finalUrl);
//
//	}
//
//	/**
