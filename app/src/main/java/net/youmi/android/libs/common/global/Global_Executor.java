...
public class Global_Executor {
	 ...
	 */
	private final static Executor mCachedThreadPool =
			Executors.newCachedThreadPool(new DefaultThreadFactory(Thread.NORM_PRIORITY - 1, null));
	...
	public static class DefaultThreadFactory implements ThreadFactory {
		...
		public Thread newThread(Runnable r) {
			...
			try {
				...
				if (DLog.isDebug) {
					if (t != null) {
						DLog.dd(t.getName(), "第%d个线程创建成功", mThreadNumber.get() - 1);
					}
				}
			}
			return t;
