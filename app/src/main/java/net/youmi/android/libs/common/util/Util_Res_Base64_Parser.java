...
public class Util_Res_Base64_Parser {
	...
	public synchronized static Bitmap decodeBitmapFromBase64(String base64) {
		try {
			if (base64 == null) {
				if (DLog.isUtilLog) {
					DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, "获取bitmap时出错，base64 == null");
				}
				return null;
			}
			...
			if (buff == null) {
				if (DLog.isUtilLog) {
					DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, "获取bitmap时出错，解析base64字符串为数组buff == null");
				}
				return null;
			}
			...
			if (DLog.isUtilLog) {
				DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, e);
			}
		}
		return null;
	}
	...
	public synchronized static NinePatchDrawable decodeNinePathchFromBase64(Context context, String bmBase64,
			String chunkBase64) {
		try {
			if (context == null || Basic_StringUtil.isNullOrEmpty(bmBase64) || Basic_StringUtil.isNullOrEmpty(chunkBase64)) {
				if (DLog.isUtilLog) {
					DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, "获取.9png时出错，参数有误");
				}
				return null;
			}
			...
			if (bitmap == null) {
				if (DLog.isUtilLog) {
					DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, "获取.9png时出错，bitmap == null");
				}
				return null;
			}
			...
			if (chunkBytes == null) {
				if (DLog.isUtilLog) {
					DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, "获取.9png时出错，chunkBytes == null");
				}
				return null;
			}
	...
	private synchronized static byte[] getNinePathChunkByteArrayFromBitmap(Bitmap bitmap) {
		try {
			if (bitmap == null) {
				if (DLog.isUtilLog) {
					DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, "解析获取.9png的chunk时出错，bitmap == null");
				}
				return null;
			}
			// 从bitmap 中获取chunk
			...
			if (chunk == null) {
				if (DLog.isUtilLog) {
					DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, "解析获取.9png的chunk时出错，chunk == null");
				}
				return null;
			}
			...
			if (!NinePatch.isNinePatchChunk(chunk)) {
				if (DLog.isUtilLog) {
					DLog.te(DLog.mUtilTag, Util_Res_Base64_Parser.class, "解析获取.9png的chunk时出错，chunk不是.9png的数据块");
				}
				return null;
			}
			return chunk;
	...
	private synchronized static String changeByteArrayToBase64String(byte[] bytes) {
		try {
			...
			// DLog.tv(DLog.mUtilTag, Util_Res_Base64_Parser.class, "原始字符串为：\n%s", src);
			// }
			// String encodeStr = Coder_Base64.encode(src);
			// if (Debug_SDK.isUtilLog) {
			// DLog.tv(DLog.mUtilTag, Util_Res_Base64_Parser.class, "base64加密后，字符串为：\n%s", encodeStr);
			// }
			// return encodeStr;
			// }
