...
public class DownloadDBHelper extends SQLiteOpenHelper implements OnDownloadStatusChangeListener,
    ...
    private boolean updateElseInsert(SQLiteDatabase db, GameDownloadInfo downloadTask) {
        try {
            ...
            if (updateAffected != 0) {// success update
                AppDebugConfig.d(AppDebugConfig.TAG_DOWNLOAD, "Success Update pn:" + downloadTask.packageName);
                return true;
            } else {
                final long newRowId = db.insert(TABLE_NAME, null, getUpdateAppContentValues(downloadTask));
                if (newRowId != -1) {
                    AppDebugConfig.d(AppDebugConfig.TAG_DOWNLOAD, "Insert NewRowId:" + newRowId + "," +
                            "pn:" + downloadTask.packageName);
                    return true;// success insert
                }
            }
            AppDebugConfig.d(AppDebugConfig.TAG_DOWNLOAD, "ERROR! Cannot Update Or Insert pn:" + downloadTask
                    .packageName);
        } catch (Exception e) {
    ...
    public boolean deleteDownloadTask(GameDownloadInfo downloadTask) {
        try {
            AppDebugConfig.d(AppDebugConfig.TAG_DOWNLOAD, "DownloadTaskToDeleteInDB:" + downloadTask);
            ...
            AppDebugConfig.d(AppDebugConfig.TAG_DOWNLOAD, String.format("Delete DownloadRecord %s: %b", rawUrl, res));
            return res;
        } catch (Exception e) {
            AppDebugConfig.w(AppDebugConfig.TAG_DOWNLOAD, e);
        }
