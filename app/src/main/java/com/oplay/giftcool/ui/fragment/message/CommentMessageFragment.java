...
public class CommentMessageFragment extends BaseFragment_Refresh<ReplyMessage> {
    ...
    public void updateData(ArrayList<ReplyMessage> data) {
        if (data == null || data.size() == 0) {
            Global.updateMsgCentralData(getContext(), KeyConfig.CODE_MSG_COMMENT, 0, null);
            mViewManager.showEmpty();
            return;
        }
        Global.updateMsgCentralData(getContext(), KeyConfig.CODE_MSG_COMMENT, 0, data.get(0).content);
