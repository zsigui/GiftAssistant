...
public class NetworkUtil {
    ...
    public static boolean printNetworkInfo(Context context) {
        ...
        if (connectivity != null) {
            ...
            AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "-------------$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$-------------");
            AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "getActiveNetworkInfo: " + in);
            NetworkInfo[] info = connectivity.getAllNetworkInfo();
            if (info != null) {
                for (int i = 0; i < info.length; i++) {
                    // if (info[i].getType() == ConnectivityManager.TYPE_WIFI) {
                    AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "NetworkInfo[" + i + "]isAvailable : " + info[i]
                            .isAvailable());
                    AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "NetworkInfo[" + i + "]isConnected : " + info[i]
                            .isConnected());
                    AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "NetworkInfo[" + i + "]isConnectedOrConnecting : "
                            + info[i].isConnectedOrConnecting());
                    AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "NetworkInfo[" + i + "]: " + info[i]);
                    // }
                }
                AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "\n");
            } else {
                AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "getAllNetworkInfo is null");
            }
        }
        return false;
    }
    ...
    public static int getConnectedTypeINT(Context context) {
        NetworkInfo net = getConnectivityManager(context.getApplicationContext()).getActiveNetworkInfo();
        if (net != null) {
            AppDebugConfig.d(AppDebugConfig.TAG_UTIL, "NetworkInfo: " + net.toString());
            return net.getType();
        }
        return -1;
    }
