...
public class StatisticsManager {
		...
		try {
			if (AppConfig.TEST_MODE && Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
				StrictMode.setThreadPolicy(new StrictMode.ThreadPolicy.Builder()
						...
						.build());
				if (AppDebugConfig.IS_DEBUG) {
					KLog.d(AppDebugConfig.TAG_WARN, "StrictMode Data init");
				}
			}
		} catch (Throwable t) {
			t.printStackTrace();
	...
	private void initUmeng(Context context, int channelId) {
		try {
			...
			MobclickAgent.startWithConfigure(new MobclickAgent.UMAnalyticsConfig(
					context, UMENG_APP_KEY, String.format("m%d", channelId)));
			...
			if (AppDebugConfig.IS_DEBUG) {
				KLog.d(AppDebugConfig.TAG_WARN, "Umeng Data init");
			}
			if (AppDebugConfig.IS_DEBUG) {
	...
	private void initTCAgent(Context context, int channelId) {
		try {
			...
			if (AppDebugConfig.IS_DEBUG) {
				KLog.d(AppDebugConfig.TAG_WARN, "Talking Data init");
			}
		} catch (Throwable t) {
