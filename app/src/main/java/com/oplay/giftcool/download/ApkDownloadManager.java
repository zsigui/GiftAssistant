    ...
    public void addDownloadTask(GameDownloadInfo appInfo) {
        ...
        if (mUrl_AppInfo.containsKey(apkUrl)) {
            ...
            if (info == null || appInfo.downloadStatus == null) {
                AppDebugConfig.v(AppDebugConfig.TAG_DOWNLOAD, "下载内容错误");
                return;
            }
            final DownloadStatus status = info.downloadStatus;
            switch (status) {
                case DISABLE:
                    AppDebugConfig.v(AppDebugConfig.TAG_DOWNLOAD, "The Task status invalid: DISABLE CAN NOT BE " +
                            "DOWNLOAD!");
                    return;
                case PENDING:
                case DOWNLOADING: {
                    AppDebugConfig.v(AppDebugConfig.TAG_DOWNLOAD, "The Task is downloading!");
                    return;
                }
                case PAUSED:
                case FAILED: {
                    AppDebugConfig.v(AppDebugConfig.TAG_DOWNLOAD, "The Task is not downloading, restart task!");
                ...
                case FINISHED: {
                    ...
                    if (apkFile != null && apkFile.exists()) {
                        AppDebugConfig.v(AppDebugConfig.TAG_DOWNLOAD, "文件已存在,直接安装");
                        appInfo.startInstall();
                        return;
                    } else {
                        AppDebugConfig.v(AppDebugConfig.TAG_DOWNLOAD, "文件不存在,重置任务,重新下载安装");
                        deleteDownloadInfo(info);
                        break;
                    }
                }
                default: {
                    AppDebugConfig.v(AppDebugConfig.TAG_DOWNLOAD, "appInfo status invalid");
                    deleteDownloadInfo(info);
                    return;
                }
            }
