...
import net.ouwan.umipay.android.fragment.LightSDKLoginFragment;
import net.ouwan.umipay.android.fragment.LoginFragment;
import net.ouwan.umipay.android.fragment.MobileLoginFragment;
import net.ouwan.umipay.android.fragment.RegisterFragment;
...
public class UmipayActivity extends FragmentActivity implements FragmentNavigationDelegate{
	...
	public void setDefaultFragment(){
		ArrayList<UmipayAccount> mAccountList = (ArrayList<UmipayAccount>) UmipayAccountManager.getInstance(getApplicationContext())
				.getNormalAccountList();
		ArrayList<UmipayCommonAccount> mCommonAccountList =  UmipayCommonAccountCacheManager.getInstance(getApplicationContext())
				.getCommonAccountList(UmipayCommonAccountCacheManager.COMMON_ACCOUNT);
		SDKCacheConfig mCacheConfig = SDKCacheConfig.getInstance(this);
		...
		}else if(mCommonAccountList != null && mCommonAccountList.size() >0){
			replaceFragmentToActivityFragmentManager(SelectCommonAccountFragment.newInstance());
		}else if (SDKCacheConfig.getInstance(this).isLightSDK()) {
			replaceFragmentToActivityFragmentManager(LightSDKLoginFragment.newInstance());
		} else if (mAccountList == null || mAccountList.size() == 0) {
			replaceFragmentToActivityFragmentManager(RegisterFragment.newInstance());
		} else if(mCacheConfig.isMobileLogin()){
			replaceFragmentToActivityFragmentManager(MobileLoginFragment.newInstance());
		}else{
			replaceFragmentToActivityFragmentManager(LoginFragment.newInstance());
		}
	}
