...
public class UmipayCommonAccountCacheManager {
    ...
    public boolean checkDate(UmipayCommonAccount account) {
        if (account == null || account.getUid() == 0 || TextUtils.isEmpty(account.getSession()) || TextUtils.isEmpty
                (account.getDestPackageName()) || TextUtils.isEmpty(account.getOriginPackageName()) || TextUtils
		        .isEmpty(account.getUserName())) {
            return false;
        }
        return true;
    }
                    ...
                    try {
                        ...
                        if (!TextUtils.isEmpty(json)) {
                            ...
                            if (packageName.equals(account.getDestPackageName())) {
                                removeItem(jsonArray, i);
                            } else if (account.getUid() == umipayCommonAccount.getUid()) {
    ...
    private void removeItem(JSONArray jsonArray, int i) throws JSONException {
    ...
    public synchronized void removeCommonAccountsByIdAndSession(UmipayCommonAccount account, int accountType) {
        if (account == null) {
            return;
        }
        String fileName = (accountType == COMMON_ACCOUNT) ? COMMON_ACCOUNT_FILE : COMMON_ACCOUNT_TOCHANGE_FILE;
            ...
            if (jsonArray != null) {
                for (int i = 0; i < jsonArray.length(); i++) {
                    try {
                        ...
                        if (!TextUtils.isEmpty(json)) {
                            UmipayCommonAccount item = new UmipayCommonAccount(json);
                            if (account.getUid() == item.getUid()
                                    && account.getSession().equalsIgnoreCase(item.getSession())) {
                                removeItem(jsonArray, i);
                            }
                        }
                    } catch (Throwable e) {
        ...
        Debug_Log.dd("removeCommonAccountByIdAndSession");
    }
