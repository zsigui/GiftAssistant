package net.youmi.android.libs.common.v2.network.httpclient;
	...
	protected void newHttpRequest() {
		...
		try {
			...
			if (httpResponse == null) {
				if (Debug_SDK.isNetLog) {
					Debug_SDK.td(Debug_SDK.mNetTag, this, "请求结果为空");
				}
			...
			try {
				if (Debug_SDK.isNetLog) {
					Debug_SDK.td(Debug_SDK.mNetTag, this, "从Entity中获取的contentLength：%d", entity.getContentLength());
				}
			...
			if (httpResponse.getStatusLine().getStatusCode() >= 200 && httpResponse.getStatusLine().getStatusCode() < 300) {
				...
				while ((len = inputStream.read(buff)) > 0) {
					if (mIsRunning) {
						...
						//						if (Debug_SDK.isNetLog) {
						//							Debug_SDK.ti(Debug_SDK.mNetTag, this, "已读取%d ....", len);
						//						}
						//						Thread.sleep(1000);
					} else {
						return;
