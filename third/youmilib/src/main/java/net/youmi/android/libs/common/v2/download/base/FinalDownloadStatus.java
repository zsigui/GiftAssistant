...
public class FinalDownloadStatus {
	 ...
	 */
	public FinalDownloadStatus(int downloadStatusCode, Throwable throwable) {
	 ...
	 * @param downloadStatusCode 下载最终状态码{@link net.youmi.android.libs.common.v2.download.base.FinalDownloadStatus.Code}
	 */
	public FinalDownloadStatus(int downloadStatusCode) {
		mDownloadStatusCode = downloadStatusCode;
	}
	 ...
	 * 获取下载最终状态码
	 *
	 * @return {@link net.youmi.android.libs.common.v2.download.base.FinalDownloadStatus.Code}
	 */
	...
	public static class Code {
		 ...
		 */
		public final static int FAILED_ERROR_HTTP_RESPONSE = 153;
		//
		//		/**
		 ...
		 */
		public final static int FAILED_ERROR_REACH_MAX_DOWNLOAD_TIMES = 155;

		/**
		 * 下载失败：下载的缓存文件路径或者最终存放路径有一个为目录，不可重试下载
		 */
		public final static int FAILED_ERROR_LOCAL_FILE_TYPE = 156;

		/**
		 * 下载失败：下载成功时，检查下载结果时发现最终下载文件无效时的错误，不可重试下载
		 */
		public final static int FAILED_ERROR_DEST_FILE_INVALID = 157;

		/**
		 * 下载失败：下载之前发现最终存储文件存在，但是不通过检验，尝试删除该文件以重新下载时，发现该文件无法被删除，因此归类为不可重试下载
		 */
		public final static int FAILED_ERROR_DEST_FILE_CANNOT_DEL = 158;

		/**
		 * 下载失败：多进程下载时，达到获取conent-length最大上线数 不可重试下载
		 */
		public final static int FAILED_ERROR_REACH_MAX_GET_CONTENT_LENGTH_TIMES_INMULTPROCESSES = 159;
	}
}
